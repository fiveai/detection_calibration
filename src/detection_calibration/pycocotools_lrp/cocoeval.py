...
class COCOeval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType='segm', lrp_size_details=False):
        ...
        self.params = Params(
            iouType=iouType, lrp_size_details=lrp_size_details)  # parameters
            ...
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    for gind, g in enumerate(gt):
                        ...
                        #################################
                        # if np.isclose(np.array([d['score']]) , np.array([0.367403119802475]))[0]:
                        #    print(f'IoU for score {0.367403119802475} -> {ious[dind, gind]}')
                        #    print(f'Det Bbox : {d["bbox"]}')

                        # if np.isclose(np.array([d['score']]) , np.array([0.4686489403247833]))[0]:
                        #    print(f'IoU for score {0.4686489403247833} -> {ious[dind, gind]}')
                        #    print(f'Det Bbox: {d["bbox"]}')
                        #################################
    ...
    def accumulate(self, p=None):
        ...
        lrp_loc = -np.ones((K, A, M))
        lrp_fp = -np.ones((K, A, M))
        lrp_fn = -np.ones((K, A, M))
        lrp = -np.ones((K, A, M))
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    if tp_num.shape[0] > 0:
                        ...
                        if tp_num[-1] > 0:
                            ...
                            idx = -1
                            lrp[k, a, m] = lrps[idx]
                            lrp_loc[k, a, m] = total_loc[idx] / tp_num[idx]
                            lrp_fp[k, a, m] = fp_num[idx] / \
                                (tp_num[idx] + fp_num[idx])
                            lrp_fn[k, a, m] = fn_num[idx] / npig
    ...
    def summarize(self):
        ...
        def _summarize(ap=1,
            ...
            if ap == 1:
                ...
                if lrp_type == 'oLRP_false_negative':
                    ...
                    typeStr = '    '
                if lrp_type == 'LRP':
                    s = self.eval['lrp'][:, aind, mind]
                    titleStr = 'LRP'
                    typeStr = '    '
                if lrp_type == 'LRP_Localisation':
                    s = self.eval['lrp_loc'][:, aind, mind]
                    titleStr = 'LRP Loc'
                    typeStr = '    '
                if lrp_type == 'LRP_false_positive':
                    s = self.eval['lrp_fp'][:, aind, mind]
                    titleStr = 'LRP FP'
                    typeStr = '    '
                if lrp_type == 'LRP_false_negative':
                    s = self.eval['lrp_fn'][:, aind, mind]
                    titleStr = 'LRP FN'
                    typeStr = '    '
        ...
        def _summarizeDets():
            ...
            stats[10] = _summarize(
                0, iouThr=.75, maxDets=self.params.maxDets[2])
            stats[11] = _summarize(
                0, iouThr=.9, maxDets=self.params.maxDets[2])
            ...
            stats[17] = _summarize(-1,
                                   ...
                                   areaRng='all',
                                   maxDets=self.params.maxDets[2],
                                   lrp_type='LRP_Localisation')
            stats[18] = _summarize(-1,
                                   iouThr=.5,
                                   areaRng='all',
                                   maxDets=self.params.maxDets[2],
                                   lrp_type='LRP_false_positive')
            stats[19] = _summarize(-1,
                                   iouThr=.5,
                                   areaRng='all',
                                   maxDets=self.params.maxDets[2],
                                   lrp_type='LRP_false_negative')
